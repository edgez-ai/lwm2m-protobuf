...
int lwm2m_ed25519_verify_signature(const uint8_t *public_key, size_t public_key_len,
    ...
    /* Convert Java BigInteger Ed25519 public key format to libsodium format.
     * 
     * Java uses BigInteger(1, rawPublicKey) which treats bytes as big-endian Y coordinate,
     * but libsodium expects little-endian Y coordinate for Ed25519 compressed points.
     * 
     * The working solution: reverse the byte order and clear the sign bit.
     */
    uint8_t libsodium_key[32];
    for (int i = 0; i < 32; i++) {
        libsodium_key[i] = public_key[31 - i];  /* Convert big-endian to little-endian */
    }
    libsodium_key[31] &= 0x7F;  /* Clear sign bit for positive X coordinate */
    
    /* Verify signature with converted key */
    if (crypto_sign_ed25519_verify_detached(signature, message,
            (unsigned long long)message_len, libsodium_key) == 0) {
        ESP_LOGD(TAG, "Ed25519 signature verification succeeded");
        return 0;
    }
